...
; Changed ctype1/2 to HPLN_TAN and HPLT_TAN
;
; Added in more things for actual data (like AIA maps)
;
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;+
; Project     : SOHO-CDS
...
pro map2fits_sunpy, map, file, err = err, byte_scale = byte_scale, verbose = verbose
  ...
  for i = 0, np - 1 do begin
    ...
    fxaddpar, header, 'ctype1', 'HPLN_TAN', 'Solar X (cartesian west) axis'
    fxaddpar, header, 'ctype2', 'HPLT_TAN', 'Solar Y (cartesian north) axis'
    ...
    ; Added in some general useful stuff from real data
    if tag_exist(map[i], 'WAVELNTH') then fxaddpar, header, 'WAVELNTH', map[i].wavelnth, 'Wavelength'
    if tag_exist(map[i], 'WAVEUNIT') then fxaddpar, header, 'WAVEUNIT', map[i].wavelnth, 'Wavelength unit'
    if tag_exist(map[i], 'TELESCOP') then fxaddpar, header, 'TELESCOP', map[i].telescop, 'Telescope Name'
    if tag_exist(map[i], 'INSTRUME') then fxaddpar, header, 'INSTRUME', map[i].instrume, 'Instrument Name'
    if tag_exist(map[i], 'DETECTOR') then fxaddpar, header, 'DETECTOR', map[i].wavelnth, 'Detector Name'
